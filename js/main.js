$(document).ready(function() {
  ...
  function createList() {
    ...
    for (i = 0; i < ingredientsList.length; i++) {
      ...
      newIngredient.addClass(
        "btn btn-success button-border my-2 mr-sm-2 ingredient-btn"
      );
  ...
  function showRecipes(from, to, random) {
    ...
    var queryURL =
      ...
      strIngredientsList +
      "&app_id=d73fa439&app_key=3ed6144109abca33960e4f39185225ef" +
      start +
      end;
    ...
    $.ajax({
      ...
      if (random == -1) {
        for (let i = 0; i < response.hits.length; i++) {
          ...
          let link = $(
            `<a href = "${response.hits[i].recipe.url}" target = "_blank" >`
          );
          ...
          for (
            let j = 0;
            j < response.hits[i].recipe.healthLabels.length;
            j++
          ) {
            if (response.hits[i].recipe.healthLabels[j] == "Vegan") {
              let vegan = $(
                '<i class="fas fa-lg fa-seedling" title = "Vegan" style="color:#96E941; padding: 2px;"></i>'
              );
              icons.append(vegan);
            }
            if (response.hits[i].recipe.healthLabels[j] == "Vegetarian") {
              let vegetarian = $(
                '<i class="fas fa-lg fa-leaf" title = "Vegetarian" style="color:#35BD78; padding: 2px;"></i>'
              );
          ...
          for (let k = 0; k < response.hits[i].recipe.dietLabels.length; k++) {
            if (response.hits[i].recipe.dietLabels[k] == "Low-Fat") {
              let lowFat = $(
                '<i class="fas fa-lg fa-arrow-alt-circle-down" title = "Low-Fat" style="color:#FF7347; padding: 2px;"></i>'
              );
              icons.append(lowFat);
            }
          }
          ...
          for (
            let l = 0;
            l < response.hits[i].recipe.ingredientLines.length;
            l++
          ) {
            let ingre = $(
              `<p> ${response.hits[i].recipe.ingredientLines[l]}</p>`
            );
      ...
      // If a user clicks to generate a random recipe...
        ...
        let recipe = $("<div style='width: 18rem;'>");
        recipe.addClass(
          "hover-fade card col-md-3 m-2 position-absolute roll-in-bottom"
        );
        let img = $("<img>");
        img.addClass("card-img-top");
        ...
        // Add link to recipe image
        let link = $(
          `<a href = "${response.hits[random].recipe.url}" target = "_blank" >`
        );
        // Append the image to the link
        link.append(img);
        ...
        for (
          let j = 0;
          j < response.hits[random].recipe.healthLabels.length;
          j++
        ) {
        ...
        for (
          let k = 0;
          k < response.hits[random].recipe.dietLabels.length;
          k++
        ) {
        ...
        for (
          let x = 0;
          x < response.hits[random].recipe.ingredientLines.length;
          x++
        ) {
          let ingre = $(
            `<p> ${response.hits[random].recipe.ingredientLines[x]}</p>`
          );
          ingrHover.append(ingre);
        }
        // Allow users to scroll through ingredients list
